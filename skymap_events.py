...
if len(argv) == 1:
    event_file_path = input('Please enter the path to the CSV list of events: ')
    fov_area = input('Please enter the survey field of view area in sq. deg: ')
    ...
    nyears_survey = argv[4]
fov_area = float(fov_area)
nyears_survey = float(nyears_survey)

NSIDE = 64
NPIX = hp.nside2npix(NSIDE)
FOV_RADIUS = np.sqrt(fov_area / np.pi)

# Read the table of events
event_table = read_event_list(event_file_path)
...
for coord in event_table['Coordinate']:
    survey_pixels = healpixel_functions.skycoord_to_HPindex(coord, NSIDE, radius=FOV_RADIUS)
